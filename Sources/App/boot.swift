        ...
        app.withPooledConnection(to: .psql) { conn -> Future<Void> in
            ...
            return practices.flatMap { practices -> Future<Void> in
                practices.map { practice in
                    let newPractice = Practice(status: practice.status, name: practice.name, owner: practice.owner, date: Calendar.current.date(byAdding: .day, value: practice.repeatAfter, to: practice.date), repeatAfter: practice.repeatAfter)
